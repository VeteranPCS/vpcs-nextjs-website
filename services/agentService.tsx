...
function combineStateValues(data: any): string[] {
  try {
    ...
    const statesLicensed = record.State_s_Licensed_in__pc
      ? record.State_s_Licensed_in__pc.split(", ")
      : [];
    const otherStates = record.Other_States__pc
      ? record.Other_States__pc.split(";")
      : [];

    // Combine the arrays and remove any duplicates
    const combinedStates = Array.from(
      new Set([...statesLicensed, ...otherStates]),
    );
...
function getStateFullNames(abbreviations: string[]) {
  try {
    ...
    return abbreviations
      .map((abbr) => {
        const fullName: string =
          stateAbbreviations[abbr as keyof typeof stateAbbreviations];
        if (!fullName) {
    ...
    console.error(
      "Error converting state abbreviations to full names:",
      error.message,
    );
    return [];
  }
...
const agentService = {
  fetchLogosList: async (): Promise<AgentDocument[]> => {
    try {
      const logos = await client.fetch<AgentDocument[]>(
  ...
  getAgentImage: async (salesforceID: string): Promise<string> => {
    try {
      const agent = await client.fetch<AgentDocument>(
        `*[_type == "agent" && salesforceID == $salesforceID][0]`,
        { salesforceID },
      );

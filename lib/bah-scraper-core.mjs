...
export function fetchPage(url, postData = null) {
...
export function toDtmoYear(year) {
  const value = String(year);
  return value.length === 4 ? value.slice(-2) : value;
}

export function toFourDigitYear(year) {
  const value = String(year);
  return value.length === 2 ? `20${value}` : value;
}

export function extractDataFromHtml(html, rankName) {
  ...
  const withDependents = Number.parseFloat(withDepsText.replace(/[$,\s]/g, ''));
  const withoutDependents = Number.parseFloat(withoutDepsText.replace(/[$,\s]/g, ''));
  const isValid =
    withDependents > 0 &&
    withoutDependents > 0 &&
...
export async function extractBAHData(year, zipCode, rankId, fetcher = fetchPage) {
  const normalizedRankId = String(rankId);
  const rankName = RANK_MAPPING[normalizedRankId];
  ...
  const postData = {
    ...
    Rank: normalizedRankId,
  };
  const postUrl = 'https://www.defensetravel.dod.mil/neorates/report/index.php';
  const html = await fetcher(postUrl, postData);
  return extractDataFromHtml(html, rankName);
}

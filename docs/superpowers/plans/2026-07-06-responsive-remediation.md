...
- [ ] **T1.4 (H3b) Dead X buttons on both Get Listed forms** (`get-listed-agents/page.tsx:102-115`, `get-listed-lenders/page.tsx:114-127`): make them `Link` to `/` (exit affordance mid-form). Check: tap exits to homepage.
- [ ] **T1.5 (H4) NMLS/brokerage text 10px → 14px.** `StatePageCityAgents.tsx:106`, `StatePageVaLoan.tsx:78`: `sm:text-[10px] text-[10px]` → `text-sm` (keep `md:text-[18px]`). Check: screenshot at 375px, legible.
- [ ] **T1.6 (H5) 400px fixed block** `PcsResourcesHowDoesWorkIt.tsx:10`: `w-[400px]` chain → `w-full max-w-[700px] px-5` (only if Phase 0 re-verification confirms; otherwise document why not).
- [ ] Phase validation sweep (drawer, nav, state page, pcs-resources at all 6 viewports) + screenshot review + commit(s) + PR 1. Codex diff gate.

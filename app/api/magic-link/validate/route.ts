...
export async function GET(request: NextRequest): Promise<NextResponse<ValidateResponse>> {
    try {
        ...
        try {
            ...
            console.error(`Error fetching ${objectType} from Attio:`, error);
            return NextResponse.json({
                success: false,
                valid: false,
        ...
        const parentRecordId = dealInfo.parent_record_id || dealInfo.parent?.record_id;
        ...
        if (dealInfo.entry_values?.stage) {
            ...
            if (Array.isArray(stageValue) && stageValue.length > 0) {
                dealStage = stageValue[0]?.status?.title || stageValue[0]?.status || 'Unknown';
        ...
        return NextResponse.json({
            ...
            agent: {
                id: agentInfo.id,
                name: agentInfo.name || `${agentInfo.first_name} ${agentInfo.last_name}`.trim(),
                email: agentInfo.email || null,
                phone: agentInfo.phone || null,
            },
            deal: {
                id: validation.deal_id,
                type: dealInfo.entry_values?.deal_type?.[0]?.value || dealInfo.entry_values?.deal_type || 'Unknown',
                stage: dealStage,
                customer_name: customerInfo?.name || `${customerInfo?.first_name || ''} ${customerInfo?.last_name || ''}`.trim() || 'Unknown',
                customer_phone: customerInfo?.phone || null,
                customer_email: customerInfo?.email || null,
                property_address: dealInfo.entry_values?.property_address?.[0]?.value || dealInfo.entry_values?.property_address || null,
                notes: dealInfo.entry_values?.notes?.[0]?.value || dealInfo.entry_values?.notes || null,
                contact_confirmed: dealInfo.entry_values?.contact_confirmed?.[0]?.value ?? dealInfo.entry_values?.contact_confirmed ?? false,

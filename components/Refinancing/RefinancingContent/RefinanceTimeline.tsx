import React from "react";

const RefinanceTimeline = () => {
  return (
    <>
      <div>
        <h2 className="text-[#292F6C] lg:text-[33px] md:text-[30px] sm:text-[25px] text-[25px] font-bold my-2">
          Timeline: When You Can Refinance
        </h2>
      </div>
      <div className="mt-3 mb-5">
        <p className="text-[#000000] roboto lg:text-[24px] md:text-[24px] sm:text-[17px] text-[17px] md:font-medium sm:font-normal font-normal">
          You can refinance using the VA Streamline IRRRL every 210 days from your first mortgage payment due date.
        </p>
      </div>
    </>
  );
};

export default RefinanceTimeline;

// scripts/v2/migrate-state-lenders-v2.ts
// Converts lender area assignments from Salesforce to State.lenders in Attio
//
// Note: This still reads from Salesforce CSVs because the cleaned area_assignments.csv
// only contains agent assignments (lender assignments are handled differently).
...
//
// Prerequisites:
// - States must exist in Attio
// - Lenders must be migrated (data/mappings/lenders.json)
...
// Lender RecordTypeId (15-char version as exported in CSV)
...
interface AreaAssignmentRow {
  Id: string;
  IsDeleted: string;
  Agent__c: string;  // Can be Agent OR Lender (Account ID)
  Area__c: string;
  Status__c: string;
}

async function migrateStateLendersV2() {
  console.log('='.repeat(60));
  console.log('Migrate State Lenders V2');
  ...
  if (!fs.existsSync(lendersMapPath)) {
    console.error('❌ Missing lenders mapping file. Run migrate-lenders-v2.ts first.');
    process.exit(1);
  }

  // Load lender mappings (Salesforce ID → Attio ID)
  ...
  // Fetch states from Attio to build state code → Attio ID map
  console.log('Fetching states from Attio...');
  const states = await attio.queryRecords('states', { limit: 100 });
  const statesMap: Record<string, string> = {};
  for (const state of states) {
    if (state.state_code) {
      statesMap[state.state_code] = state.id;
    }
  }
  console.log(`Found ${Object.keys(statesMap).length} states in Attio`);
  console.log();

  // Read Salesforce CSVs (still needed for lender-to-state relationships)
  ...
  console.log(`Loaded ${accounts.length} accounts from Salesforce`);
  console.log(`Loaded ${areas.length} areas from Salesforce`);
  console.log(`Loaded ${assignments.length} area assignments from Salesforce`);
  console.log();

  // Build lender account ID set
  ...
  for (const assignment of lenderAssignments) {
    ...
    // Get Attio lender ID from v2 mapping
  ...
  for (const [stateCode, lenderSet] of Object.entries(stateLenders)) {
    ...
    if (!attioStateId) {
      console.error(`❌ State ${stateCode} not in Attio`);
      errorCount++;
      continue;
    }
...
migrateStateLendersV2().catch(error => {

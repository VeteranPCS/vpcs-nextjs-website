...
export async function sendOpenPhoneMessage(params: OpenPhoneMessageParams) {
  ...
  try {
    ...
    console.log(
      `Received response from OpenPhone API with status: ${response.status}`,
    );
    ...
    if (!response.ok) {
      ...
      console.error(
        `OpenPhone API request failed with status: ${response.status}`,
      );
      throw new Error(
        `OpenPhone API error: ${response.status} - ${JSON.stringify(responseData)}`,
      );
    }
    ...
    console.error(
      "Error sending OpenPhone message for %d recipients:",
      to.length,
      error,
    );
    throw error;
